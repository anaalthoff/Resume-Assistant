Área de interesse: Frontend
Nível de experiência: Pleno
Preferências de trabalho: Remoto
Localização: Remoto
Soft skills: Pensamento crítico, curiosidade técnica, flexibilidade, resiliência e comunicação orientada a propósito e ação.
Objetivo de carreira: Crescimento técnico
Habilidades atuais: JavaScript, TypeScript, HTML, CSS, Vue
Funções alvo: Engenheiro Frontend, Desenvolvedor UI, Desenvolvedor React
Concluído: true